...
DROP TRIGGER IF EXISTS set_mood_emoji_trigger ON public.mood_logs;
CREATE TRIGGER set_mood_emoji_trigger
  BEFORE INSERT OR UPDATE ON public.mood_logs
  FOR EACH ROW
  EXECUTE FUNCTION public.set_mood_emoji();

-- Create unique index to prevent multiple mood entries per day per user
DROP INDEX IF EXISTS unique_user_date_mood_idx;
CREATE UNIQUE INDEX unique_user_date_mood_idx 
ON public.mood_logs (user_id, DATE(logged_at));
